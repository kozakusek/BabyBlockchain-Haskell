...
mineBlock miner parent txs = Block (fst $ fromJust $ find (validNonce . fst) blocks) txs
  where
    c = coinbaseTx miner
    t = treeHash $ buildTree (c:txs)
    start = (BlockHeader parent c t (hash (0::Word32)), 0::Word32)
    blocks = iterate (\(b, r) -> (b { nonce = hash (r + 1) } , r + 1)) start

genesis, block0, block1, block2 :: Block
genesis = block0
...
verifyChain = foldr ((=<<) . verifyBlock) (Just 0)
...
mineTransactions miner parent txs = (b, f <$> txs)
  where
    b = mineBlock miner parent txs
    f = TxReceipt (hash b) . fromJust . flip buildProof (buildTree (coinbase (blockHdr b):txs))
